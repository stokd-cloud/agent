<!-- DSH TUI 0.10.0-beta.4 donor documentation. For Stokd Agent use ../README.md. -->
<p align="center">
  <img src="assets/logo.svg" alt="dsh-TUI - DeepSeek Harness terminal interface" width="560">
</p>

<p align="center">
  <a href="../README.md">Stokd Agent</a> · <strong>English</strong>
</p>
...
> [Architecture and limitations](architecture.en.md) before relying on its
...
[Running dsh-TUI in VS Code](vscode.en.md).

See [Getting started](getting-started.en.md) for profile composition,
...
[Getting started](getting-started.en.md#migrate-from-the-former-package).
...
| Topic | Contents |
| --- | --- |
| [Getting started](getting-started.en.md) | Prerequisites, installation, startup, profile lifecycle, source development |
| [Configuration](configuration.en.md) | Cordis overrides, fields, agent presets, MCP, environment variables |
| [Themes](themes.en.md) | Built-in themes, background detection, static JSON and npm plugin themes, validation |
| [Interaction and commands](interaction.en.md) | Keyboard, mouse, questionnaires, slash commands, session workflows |
| [Architecture and limitations](architecture.en.md) | Runtime path, rendering, persistence, security boundary, known limitations |
| [VS Code guide](vscode.en.md) | Running dsh-tui in the VS Code integrated terminal; the `dsh-tui-vscode` companion extension offers an experience almost identical to the official Claude Code extension (on the Marketplace) |
| [Contributing](contributing.en.md) | Contribution workflow, repository map, build artifacts, verification matrix, change rules |
...
The complete bilingual index is [`docs/README.md`](README.md).
...
- **Agent presets**: four official agent modes (`standard` / `ptc` / `minimal` / `cordis`)
  ...
  See [Configuration](configuration.en.md#agent-preset).
...
- **MCP**: servers are mounted via `@deepseek-ai/dsh-mcp-client`, with tools registered as
  ...
  See [Configuration](configuration.en.md#mcp).
...
[architecture guide](architecture.en.md) for module boundaries and
performance details.
...
See [Architecture and limitations](architecture.en.md) for the complete list of
...
See [Permissions and security boundary](architecture.en.md#permissions-and-security-boundary)
for details.
